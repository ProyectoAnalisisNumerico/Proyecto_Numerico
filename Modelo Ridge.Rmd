...
```{r}
# Preparar datos
X_matrixR <- as.matrix(datos_finalR %>% select(-Materia_Organica))
y_vectorR <- as.numeric(datos_finalR$Materia_Organica)
...
ridge_lambda  # mejor λ encontrado
```
```{r}
# Coeficientes del modelo
ridge_coef <- as.matrix(coef(ridge_model, s = "lambda.min"))
ridge_coef
```

```{r}
# Error medio de validación cruzada
ridge_error <- ridge_model$cvm[which.min(ridge_model$cvm)]
ridge_error
```

```{r}
# Predicciones y R²
pred_ridge <- predict(ridge_model, s = ridge_lambda, newx = X_matrixR)
r2_ridge <- cor(y_vectorR, pred_ridge)^2
r2_ridge
```

```{r}
# Visualizar importancia relativa de variables
barplot(sort(ridge_coef[,1]),
        main = "Importancia de variables - Modelo Ridge",
        las = 2, cex.names = 0.7,
