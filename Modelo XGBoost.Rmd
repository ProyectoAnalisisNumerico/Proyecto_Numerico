...
#X_scaledL[vars_sesgadas] <- lapply(X_scaledL[vars_sesgadas], winsorizar)
#head(X_scaledL)
...
#boxplot(
#  X_scaledL,
#  main = "Distribución de variables químicas (tras Winsorizado y Escalado)",
#  las = 2,             # Etiquetas del eje X en vertical
#  cex.axis = 0.8,      # Tamaño de números del eje
#  cex.names = 0.9,     # Tamaño de nombres de variables
#  col = "skyblue",     # Color de cajas
#  border = "gray40",   # Color de bordes
#  outline = TRUE       # Mostrar outliers remanentes
#)
```
...
# Preparar matrices en formato requerido por XGBoost

dtrain <- xgb.DMatrix(data = as.matrix(x_train), label = y_train)
dtest  <- xgb.DMatrix(data = as.matrix(x_test), label = y_test)

# Definir parámetros base del modelo XGBoost

parametros <- list(
booster = "gbtree",         # tipo de modelo
objective = "reg:squarederror",  # regresión continua
eta = 0.03,                 # tasa de aprendizaje
max_depth = 8,              # profundidad de los árboles
subsample = 0.8,            # muestreo aleatorio de observaciones
colsample_bytree = 0.9,     # muestreo aleatorio de variables
min_child_weight = 6,  # mínimo peso para dividir nodos
gamma = 0                   # regularización adicional
)
...
params = parametros,
data = dtrain,
nrounds = 5000,                     # número máximo de iteraciones
watchlist = list(train = dtrain, test = dtest),
early_stopping_rounds = 30,        # detenerse si no mejora el error
print_every_n = 20,                # mostrar progreso cada 20 iteraciones
verbose = 1
)
...
mse_xgb  <- mean((pred_xgb_test - y_test)^2)
rmse_xgb <- sqrt(mse_xgb)
mape_xgb <- MAPE(pred_xgb_test, y_test) * 100

cat("Error cuadrático medio (MSE):", round(mse_xgb, 4), "\n")
cat("Raíz del error cuadrático medio (RMSE):", round(rmse_xgb, 4), "\n")
cat("MAPE:", round(mape_xgb, 2), "%\n")
```

```{r}
# Transformar las predicciones y valores reales a escala original

pred_xgb_real <- exp(pred_xgb_test) - 1
real_xgb      <- exp(y_test) - 1

comparacion_xgb <- data.frame(
Real = round(real_xgb, 3),
Predicho = round(pred_xgb_real, 3)
)

head(comparacion_xgb, 15)
```
...
importancia_xgb <- xgb.importance(model = xgb_model)
xgb.plot.importance(importancia_xgb, top_n = 15, rel_to_first = TRUE,
main = "Importancia de las variables en XGBoost")
```

```{r}
library(ggplot2)

df_eval_xgb <- data.frame(
Real = real_xgb,
Predicho = pred_xgb_real
)

ggplot(df_eval_xgb, aes(x = Real, y = Predicho)) +
geom_point(color = "steelblue", alpha = 0.6) +
geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
labs(title = "Comparación entre valores reales y predichos (XGBoost)",
x = "Materia orgánica real",
y = "Materia orgánica predicha") +
theme_bw()
```
